...
def demo_pygame(file_name,frm):
	...
	# loading the sound files in different sound formats of mixer and main background sound in mixer stream mixer.init()
    sound = mixer.Sound('./sounds/wind1.ogg')
    sound.play(-1)
	# setting up an environment for shifting the map while exiting from the map
    portal1 = pygame.Rect(7*16,1*16,64,80) 
    ...
    while running:
        ...
        if(pygame.Rect.colliderect(hero.rect,portal1) ):
            sound.stop()
            portal=True
            portal_num=0
        ...
        pygame.display.flip()
	# calling next part of task to be completed 
    if (portal==True):
...
def create_hero(start_pos_x, start_pos_y):
    ...
    """
    image = pygame.image.load('./images/head.png')
